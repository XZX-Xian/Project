...
<div class="x-body">
    <div class="layui-row">
        <input type="text" name="username" placeholder="请输入用户名" <%--autocomplete="off"--%> size="20" id="fuzzy"
               style="height: 40px;">
    ...
    <table class="layui-table">
        ...
        <c:forEach items="${list}" var="in">
            <tr>
                ...
                <td>${in.date}</td>
                <td class="td-manage">
                    <a title="编辑" onclick="x_admin_show('编辑','admin-edit.jsp?account='+${in.account})" href="javascript:;">
                        <i class="layui-icon">&#xe642;</i>
                    </a>
                    <a title="删除" onclick="member_del(this,'${in.account}')" href="javascript:;">
                        <i class="layui-icon">&#xe640;</i>
                    </a>
                </td>
            </tr>
...
<script>
    ...
    $(function () {
        ...
        fuzzy=function () {
            ...
            var json={"name":name};
            $("#listall").html("");
            $.getJSON("http://localhost:8080/Project_war_exploded/adminlist",json,function (data) {
                var list="";
                $("#listall").html(list);
        ...
        member_del = function (obj, id) {
            layer.confirm('确认要删除吗？', function (index) {
                //发异步删除数据
                var json = {"account": id};
                $.getJSON("http://localhost:8080/Project_war_exploded/admindel", json, function (data) {
                    if (data > 0) {
        ...
        delAll = function (argument) {
            ...
            layer.confirm('确认要删除吗？' + date, function (index) {
                ...
                location.href="http://localhost:8080/Project_war_exploded/admindelall?account="+date;
